...
TODO:
programmation objet
ameliorer ouverture  : trouveOuverture remplace par le nouveau
"""
		...
		if trouve:
			...
			if isBlunder:
				if player.lower() in game.headers["White"].lower():
					...
					t={}
					t[0]='BLANC'
					t[1]=min(numeroCoup,MAX_BLUNDER_LEVEL) # ce qui nous interesse c'est surtout les erreurs dans les debuts
					...
					t={}
					t[0]='NOIR'
					t[1]=min(numeroCoup,MAX_BLUNDER_LEVEL)
					hclefgame[pgn_string]=t
...
fichiersouv = ['opening/a.tsv', 'opening/b.tsv', 'opening/c.tsv','opening/d.tsv','opening/e.tsv']

db = {}
ret = 0


for arg in fichiersouv:
	ret = max(ret, identify_opening(arg, db))

chessboard = chess.Board()
chessboard.push_san("e4")
chessboard.push_san("d5")
chessboard.push_san("exd5")
chessboard.push_san("Nf6")
cols=findInDb(db,chessboard.fen())
print(cols)

chessboard = chess.Board()
chessboard.push_san("e4")
chessboard.push_san("e5")
chessboard.push_san("Nf3")
chessboard.push_san("Nc6")
chessboard.push_san("Bb5")
chessboard.push_san("a6")
cols = findInDb(db, chessboard.fen())
print(cols)
# main start
debut=time.time()
